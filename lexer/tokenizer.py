...
class Lexer:
    ...
    def scan(self) -> Token:
        ...
        while self._string[self._pos].isspace():
            self._pos += 1
        ...
        if char == '"':
            return Token("literal", self._scanstring())
    ...
    def _scansymbol(self) -> str:
        ...
        start = self._pos
        while self._pos + 1 < len(self._string) and (self._string[self._pos + 1] in "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789_"):
            self._pos += 1
    ...
    def _scannumber(self) -> str:
        ...
        start = self._pos
        while self._pos + 1 < len(self._string) and self._string[self._pos + 1].isnumeric():
            self._pos += 1
        return self._string[start: self._pos + 1]
